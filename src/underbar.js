(function() {
  ...
  _.last = function(array, n) {
    return n === undefined ? array.slice(-1).pop() : array.slice(Math.max(array.length - n, 0), array.length);
  };
  ...
  _.filter = function(collection, test) {
    ...
    _.each(collection, function(item) {
      if (test(item) == true) {output.push(item)};
    });
    return output;
  };
  ...
  _.reject = function(collection, test) {
    var output = [];
    _.each(collection, function(item) {
      if (test(item) == false) {output.push(item)};
    });
    return output;
  ...
  _.uniq = function(array) {
    ...
    _.each(array, function(item) {
      if (_.indexOf(output, item) == -1) {output.push(item);};
    });
    return output;
  };
  ...
  */    
  ...
  _.shuffle = function(array) {
      var copied_array = array.slice(); 
       var shuffled_new_array = [];

       for (var i = 0; i < array.length; i++ ) {
        shuffled_new_array.push(copied_array.splice(Math.random() * copied_array.length, 1)[0]);    
       }
       return shuffled_new_array;            
  };
